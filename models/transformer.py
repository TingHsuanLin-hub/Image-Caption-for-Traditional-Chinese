...
class Swin_V2_Caption_Encoder(nn.Module):
    def __init__(self, pre_trianed = True, fine_tune = False, hidden_dim=512):
        ...
        self.model = swin_transformer_v2.SwinTransformerV2(img_size=256,
                    ...
                    ape=False,
                    patch_norm=True,
                    use_checkpoint="./models/swinv2_tiny_patch4_window8_256.pth",
                    pretrained_window_sizes=[0, 0, 0, 0])
    
        self.linear = torch.nn.Linear(768, hidden_dim)

        checkpoint = torch.load("./models/swinv2_tiny_patch4_window8_256.pth")
    ...
    def forward(self, x):
        bs, c, h, w = x.shape
        x =  self.model(x) # B C*L
        x = x.reshape(bs, 64, 768) # B C L
        x = self.linear(x) # B C 512
...
class Transformer(nn.Module):
    ...
    def __init__(self, config, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        self.swin_v2 = Swin_V2_Caption_Encoder(fine_tune=True, hidden_dim = config.hidden_dim)
    ...
    def forward(self, src, tgt, tgt_mask):
        ...
        memory = self.swin_v2(src)
        # print("After swin transformer: ", memory.shape)
        ...
        # print("After encoder transformer: ", memory.shape)
        hs = self.decoder(tgt, memory, tgt_key_padding_mask=tgt_mask, query_pos=query_embed,
                          tgt_mask=generate_square_subsequent_mask(len(tgt)).to(tgt.device))
